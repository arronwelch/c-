...
void InitMap(map<string, vector<string>> &svmap, ifstream &ifile)
{
	...
	while (getline(ifile, textline, '\n'))
	{
		...
		if (!prevPos)
			familyName = textline.substr(prevPos, nowPos - prevPos);
		else if (prevPos < textSize)
...
{
	for (map<string, vector<string>>::iterator itr = svmap.begin(), mapEnd=svmap.end();
			itr != mapEnd; ++itr)
	{
		ofile << "The " << itr->first << " family ";
		if (itr->second.empty())
			ofile << "has no children.\n";
		else
		{
			ofile << "has " << itr->second.size() << " children: ";
			for (vector<string>::iterator itrvec = itr->second.begin(), vecEnd = itr->second.end();
					itrvec != vecEnd; ++itrvec)
				ofile << *itrvec << ' ';
			ofile << '\n';
		}
	}
...
void UserQuery(map<string, vector<string>> &svmap)
{
	...
	if (i >= svmap.size())
		cout << "Sorry, the " << queryName << " family is not found.\n";
}
...
int main()
{
	...
	if (!readFile.is_open() || !writeFile.is_open()) {
		cerr << "Sorry, the file fails to read!\n";
		return -1;
	}
